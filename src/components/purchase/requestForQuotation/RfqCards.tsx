...
interface RfqCardsProps {
  requestForQuotations: RequestRow[];
}
...
function RfqCard({ request, index }: RfqCardProps) {
...
export function RfqCards({ requestForQuotations }: RfqCardsProps) {
  return (
    <motion.div
    ...
    >
      {/* Request for Quotations Cards Grid */}
      {requestForQuotations.length > 0 ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 py-6">
          {requestForQuotations.map((request, index) => (
            <RfqCard key={request.id} request={request} index={index} />
          ))}
        </div>
      ) : (
        <motion.div
