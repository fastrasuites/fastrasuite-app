...
export default function EditMaterialConsumptionPage() {
  ...
  const { data: apiData, isLoading: isFetchingReq, isError: isFetchError } = useGetMaterialConsumptionQuery(Number(reqId), {
  ...
  useEffect(() => {
    if (apiData) {
      ...
      setSelectedProjectId(data.project ? String(data.project) : "");
      
      const phaseStr = data.phase || data.phase_details?.id;
      if (phaseStr) setSelectedPhaseId(String(phaseStr));
      
      const actStr = data.activity || data.activity_details?.id;
      if (actStr) setSelectedTaskId(String(actStr));

      setSelectedLocationId(data.location ? String(data.location) : "");
      
      if (data.date_consumed) setDateConsumed(data.date_consumed);
      if (data.notes) setNotes(data.notes);

      if (data.lines && data.lines.length > 0) {
        setLines(data.lines.map((l: any) => ({
          id: l.id ? String(l.id) : Date.now().toString() + Math.random(),
        ...
        setLines([{ id: Date.now().toString(), product: "", quantity: "", unit_cost: "", total_cost: 0 }]);
      }
    }
  }, [apiData]);
  ...
  const [updateRequest, { isLoading: isUpdating }] = useUpdateMaterialConsumptionMutation();
  ...
  const executeSubmission = async () => {
    try {
      ...
      // Call API
      await updateRequest({ id: Number(reqId), body: payload }).unwrap();
  ...
  return (
    <PageGuard application="inventory" module="materialconsumption">
      ...
      <header className="w-full border-b border-gray-100 bg-white sticky top-0 z-30 shadow-none">
        <div className="max-w-2xl mx-auto px-4 h-16 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <button
              onClick={() => router.push(`/inventory/operation/material-consumption/${reqId}`)}
              className="p-1 rounded-lg hover:bg-gray-50 transition-colors"
              aria-label="Back"
            >
              <ArrowLeft size={20} className="text-gray-600" />
            </button>
            <h1 className="text-lg font-bold text-gray-800">Edit Material Consumption</h1>
          </div>
        </div>
      </header>
      ...
      <main className="max-w-2xl mx-auto px-4 pt-4 space-y-4">
        {isFetchingReq ? (
          <div className="p-8 space-y-4 bg-white rounded-xl border border-gray-200">
            <Skeleton className="h-10 w-full" />
            <Skeleton className="h-32 w-full" />
          </div>
        ) : isFetchError || !apiData ? (
          <div className="p-12 text-center text-red-500 font-medium bg-white rounded-xl border border-gray-200">
            Failed to load material consumption request.
          </div>
        ) : (
          <>
            {/* Request Details */}
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 p-4 z-40">
        <div className="max-w-2xl mx-auto flex gap-3">
          ...
          <Button
            ...
            disabled={isUpdating || !selectedTaskId || isFetchingReq}
          >
            {isUpdating ? <Loader2 className="w-4 h-4 mr-2 animate-spin" /> : null}
            {isUpdating ? "Saving Draft..." : "Save Draft Requisition"}
          </Button>
        </div>
      </div>
      ...
      <StatusModal
        ...
        onClose={() => {
          setModalType(null);
          router.push(`/inventory/operation/material-consumption/${reqId}`);
        }}
        title="Requisition Updated!"
        message="Your Material Consumption request has been updated successfully."
        type="success"
